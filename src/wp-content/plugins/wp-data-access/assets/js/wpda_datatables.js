	...
	*/
	...
	var ajaxOrLocalData = { 
		ajax: {
			...
			data: function(data) {
				const searchKeyword = jQuery("#searchAllValue").val();
				const filterValue = jQuery( '#searchFilterValue').val();
				...
				data.pub_id = pub_id;
				data.length =searchKeyword || filterValue ? 10 : 0;
				data.filter_field_name = filter_field_name;
				data.filter_field_value = filter_field_value;
				data.nl2br = nl2br;

				if (searchKeyword || filterValue) {
					jQuery("#" + table_name + pub_id).show();
					jQuery(".dataTables_paginate").show();
				} else {
					jQuery("#" + table_name + pub_id).hide();
	...
	var jQueryDataTablesDefaultOptions = {
		...
		initComplete: function( settings, json ) {
			...
			// Setup - add a filter select input for searching tool
			jQuery("#" + table_name + pub_id + "_length").html( '<input type="text" id="searchFilterValue" placeholder="Search filter value" />' );

			jQuery("#" + table_name + pub_id + "_length").append('<select id="searchFilterName">')
			...
			jQuery("#" + table_name + pub_id + "_length").append('<input type="button" id="searchFilterButton" value="Find" class="searchFilter_button" />')

			jQuery("#" + table_name + pub_id + "_length").insertAfter("#" + table_name + pub_id + "_filter");
			jQuery("#" + table_name + pub_id + "_filter").append('<input type="text" id="searchAllValue" placeholder="Search all value" />')
			...
			jQuery( '#searchAllButton').on( 'click', function () {
				const searchValue = jQuery("#searchAllValue").val();
				if(searchValue) {
					table
						.search( searchValue )
			...
			table.columns().eq(0).each( function (colIdx) {
				jQuery( 'input', table.column(colIdx).footer() ).on( 'keyup change', function () {
					...
					filter_field_value = ""
					jQuery("#" + table_name + pub_id + " tfoot th").each( function () {
						let name = jQuery("#" + table_name + pub_id + " tfoot th").eq( jQuery(this).index() ).attr( "data-column_name_search" )	
						let value = jQuery("#" + name).val();
						if(value !== "") {
							filter_field_name === "" ? (filter_field_name = name) : (filter_field_name += ", " + name)
							filter_field_value === "" ? (filter_field_value= "%" + value + "%") : (filter_field_value += ", %" + value + "%")
						}
					} );
					table
						.column( colIdx )
	...
	jQuery("#" + table_name + pub_id + " tfoot th").each( function () {
		var title = jQuery("#" + table_name + pub_id + " thead th").eq( jQuery(this).index() ).text();
		var name = jQuery("#" + table_name + pub_id + " thead th").eq( jQuery(this).index() ).attr( "data-column_name_search" )	
		jQuery(this).html( '<input type="text" id="' + name +'" placeholder="Search '+title+'" />' );
	} );
