...
function erp_hr_leave_insert_request( $args = [] ) {
    ...
    $defaults = [
        ...
        'taken_year'   => erp_current_datetime()->format( 'Y' ),
        'days_in_federation'    => 0.0,
        'days_out_federation'   => 0.0,
        'contact_no'   => '',
        'address_on_leave' => '',
        'substitute_required' => 0,
        'substitute_type' => Leave_Request::get_substitute_type_enum()[0],        
        'reason'       => '',
        'status'       => 0,
    ];
    ...
    if ( $leaves ) {
        $request = apply_filters( 'erp_hr_leave_new_args', [
            ...
            'taken_year'  => $args['taken_year'],
            'days_in_federation'    => absint($args['days_in_federation']),
            'days_out_federation'   => absint($args['days_out_federation']),
            'contact_no'   => $args['contact_no'],
            'address_on_leave' => wp_kses_post($args['address_on_leave']),
            'substitute_required' => absint($args['substitute_required']),
            'substitute_type' => $args['substitute_type'],
